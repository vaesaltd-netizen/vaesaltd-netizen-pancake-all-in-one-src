(function () {
          ...
          setTimeout(function () {
            ...
            if (session.pancakeTags && session.pancakeTags.length > 0) {
              ...
              if (session.tagSelections) {
                ...
                if (session.tagSelections.error) tagSelections.error = session.tagSelections.error;
              }
              ...
              if (window._tagMultiSelects) {
                ...
                if (ms.error) ms.error.populateDropdown(session.pancakeTags);
                ...
                if (ms.error) ms.error.updateChips();
              }
              ...
              getEl("p-tag-error-section").style.display = "";

              // Cập nhật nút quét tag
              var scanTagBtn = getEl("p-scan-tags");
              if (scanTagBtn) {
                scanTagBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:16px;height:16px"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg> Quét lại thẻ (' + session.pancakeTags.length + ' thẻ)';
              }
            }
  ...
  function renderPageList(pagesData) {
    ...
    listEl.querySelectorAll(".page-item").forEach(function (pageItem) {
      pageItem.onclick = function () {
        ...
        var scanTagBtn = getEl("p-scan-tags");
        if (scanTagBtn) scanTagBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:16px;height:16px"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg> Quét thẻ Pancake';
  ...
  function setupTagUI() {
    ...
    var sourceMS = initTagMultiSelect("", "Chọn thẻ...", "source");
    var removeMS = initTagMultiSelect("remove", "Chọn thẻ loại trừ...", "remove");
    var untagMS = initTagMultiSelect("untag", "Chọn thẻ cần xóa...", "untag");
    var addMS = initTagMultiSelect("add", "Chọn thẻ cần gắn...", "add");
    var errorMS = initTagMultiSelect("error", "Chọn thẻ gắn khi lỗi...", "error");

    // Expose để restore session có thể access
    window._tagMultiSelects = { source: sourceMS, remove: removeMS, untag: untagMS, add: addMS, error: errorMS };

    // Sync: Có chứa thẻ và Loại trừ thẻ không được chọn cùng tag
    ...
    window._syncConflictDropdowns = function () {
      ...
      // Có chứa thẻ ↔ Loại trừ thẻ: không chọn chung
      ...
      // Xóa thẻ ↔ Gắn thẻ: không chọn chung
    ...
    scanTagBtn.onclick = function () {
      ...
      scanTagBtn.innerHTML = '<span class="spinner"></span> Đang quét thẻ...';

      PancakeAPI.getTags(appState.sel.id, token, function (err, tags) {
        scanTagBtn.disabled = false;
        if (err || !tags || tags.length === 0) {
          scanTagBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:16px;height:16px"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg> Quét thẻ Pancake';
        ...
        scanTagBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:16px;height:16px"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg> Quét lại thẻ (' + tags.length + ' thẻ)';
      });
    };
  }
  ...
  function filterByExcludeTags(result, excludeTagIds, pancakeToken, callback) {
    ...
    function processNextTag() {
      if (tagsProcessed >= totalTags) {
        ...
        console.log("[Vaesa] Loại trừ thẻ: " + before + " → " + filtered.length + " (loại " + (before - filtered.length) + " KH)");
  ...
  function handleImagesSelect() {
    ...
    filesArr.forEach(function (imgFile, idx) {
      ...
      var isVideo = imgFile.type && imgFile.type.startsWith("video/");
      var imgEl;
      if (isVideo) {
        ...
        imgEl.onloadeddata = function () { URL.revokeObjectURL(imgEl.src); };
      } else {
        imgEl = document.createElement("img");
        imgEl.src = URL.createObjectURL(imgFile);
        imgEl.onload = function () { URL.revokeObjectURL(imgEl.src); };
      }
    ...
    VaesaSender.start(
      ...
      msgFiles,
      {
        ...
        pancakeTagConfig: (function () {
          ...
          var errorIds = (tagSelections.error || []).map(function (t) { return t.id; });
          if (addIds.length === 0 && removeIds.length === 0 && errorIds.length === 0) return null;
  ...
  function renderTemplateTab() {
    templatesContainer.innerHTML =
      ...
      '    <div class="img-picker"><div class="img-drop" id="tmpl-img-drop"><input type="file" id="tmpl-att" accept="image/*,video/*" multiple style="display:none"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg> <span>Chon anh/video</span></div><div class="img-thumbs" id="tmpl-img-thumbs"></div></div>' +
